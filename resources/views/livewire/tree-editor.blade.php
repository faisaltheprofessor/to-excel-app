...
<div class="h-screen overflow-hidden flex flex-col">
    ...
    <div class="px-6 pb-24 grow min-h-0">
        {{-- SINGLE scroll container lives here --}}
        <flux:card class="h-full overflow-auto" data-tree-root>
            <div class="pr-2">
                {{-- Bottom padding ensures whitespace below last item, so nothing hides under the fixed bar --}}
...
{{ $generatedJson }}
    ...
    <flux:modal name="confirm-move" class="min-w-[32rem]">
        <div class="space-y-5">
            <div>
                ...
                @else
                    {{-- Fall B: unterschiedliche Eltern (oder "into") → zeige volle Pfade --}}
                    <flux:text class="mt-2 space-y-2">
                        ...
                        <div class="text-sm space-y-1">
                            ...
                            <div>
                                <span class="font-medium">Neuer Pfad (Elternknoten):</span>
                                {{ $pendingPosition === 'into' ? $pendingNewParentPathStr : $pendingNewParentPathStr }}
                            </div>
    ...
    <script>
        (function () {
            // --- Drag & Drop helpers (safe tokenization + sibling reorder + confirmation) ---
            ...
            // Decide pos (before/into/after) based on cursor Y inside a row
            ...
            // Allow drops (on rows and drop zones)
            document.addEventListener('dragover', (e) => {
                ...
                // live highlight when hovering a row
                if (row) {
                    const pos = posFromRow(e, row);
                    // remove prior rings, then add the appropriate one
            ...
            // Visual hints (enter/leave for drop strips and rows)
            ...
            // DROP handler -> call preparePendingMove() then open modal
            ...
            // Lock page scroll (only the card scrolls)
            document.documentElement.classList.add('overflow-hidden');
            document.body.classList.add('overflow-hidden');

            // Excel download handler (bestehend)
