<div class="flex h-[calc(100vh-4rem)] overflow-hidden w-3/4 mx-auto"
     x-data="{ openPanel: @entangle('selectedId') }">
    <div class="h-[90%] flex flex-col transition-all duration-200 {{ $selectedId ? 'w-1/2' : 'w-full' }}"
        ...
        <div class="flex-1 overflow-x-auto"
            ...
            <div class="flex gap-4 min-w-max h-full p-4 select-none">
                @foreach ($this->columns as $col)
                    <div class="w-80 flex-shrink-0 rounded-lg bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 flex flex-col h-full"
                         x-data="{ status: '{{ $col['key'] }}', over: false }"
                         @dragover.prevent="over = true"
                         @dragleave="over = false"
                         @drop.prevent="over = false; $wire.moveTicket(parseInt($event.dataTransfer.getData('ticket-id')), status)">
                        <div class="px-4 py-3 flex justify-between items-center border-b border-zinc-200 dark:border-zinc-700 rounded-t-lg" :class="over ? 'bg-blue-50/60 dark:bg-blue-900/20' : ''">
    ...
    <div class="border-l border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 overflow-y-auto w-1/2 {{ $selectedId ? '' : 'hidden' }}"
         x-cloak
         :class="openPanel ? 'block' : 'hidden'"
